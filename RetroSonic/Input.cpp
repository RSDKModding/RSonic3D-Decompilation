...
LPDIRECTINPUTDEVICEA DirectInputDevice;
LPDIRECTINPUTA DirectInput;

int InitInputDevice(HWND hWnd, HINSTANCE hInst)
{
    if (DirectInputCreate(hInst, DIRECTINPUT_VERSION, &DirectInput, NULL))
        return S_OK;

    InputDeviceState = DirectInput->CreateDevice(GUID_SysKeyboard, &DirectInputDevice, 0);
    if (InputDeviceState)
        return S_OK;

    InputDeviceState = DirectInputDevice->SetDataFormat(&c_dfDIKeyboard);
    InputDeviceState = DirectInputDevice->SetCooperativeLevel(hWnd, 0x00000002 | 0x00000004);
    DirectInputDevice->Acquire();

    InputDeviceState = DirectInputDevice->GetDeviceState(sizeof(keys), keys);
...
void CheckKeyPress(InputData *input, byte start, byte end)
{
    ...
    byte scancodes[] = { 1, 2, 3, 4, 5, 6, 7, 8 };
